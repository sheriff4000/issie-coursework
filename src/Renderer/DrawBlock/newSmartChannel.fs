...
let getStartSegments (model: Model) (channel: BoundingBox) : SegmentId
...
let smartChannelRoute //spaces wires evenly 
    ...
    ///     - getStartSegment -> returns the starting wire segment of the channel entry
    ///     - getEndSegment -> returns the wire segment ending the channel exit
    ///     - straightenWire -> straightens the wire in between the start and end segments
    ///     - getWireConstraints -> determines the highest y and lowest y each wire can be in the channel
    ///     - getWireOrder -> gets the order the wires should be in based on the wire constraints
    ///     - getWireSpacings -> determines the y value each straightened channel section should be at
    ///     - moveWires -> moves the straightened sections to these y values.
